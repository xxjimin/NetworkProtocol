...
void L2_LLI_dataIndFunc(uint8_t srcId, uint8_t* dataPtr, uint8_t size, uint8_t BR)
{
    ...
    if (msgType == BCAST)
    {
        if (rcvdRssi >= RSSI_THRESHOLD)
        {
            // QUERY_LIKE 전송
            uint8_t msg[64];
            uint8_t payload[1] = {rcvdSrc};  // 또는 myL2ID로 수정
            uint8_t msgSize = L2_msg_encodeData(msg, payload, 0, 1, 1);
            L2_LLI_sendData(msg, msgSize, rcvdSrc);  // BCAST 보낸 쪽에 답장
        }
    }
    ...
    if ((float)rand()/RAND_MAX > L2_LLI_PKT_LOSS)
    {
        ...
        if (msgType == ANS_LIKE)
        {
            uint8_t liked = rcvdData[L2_MSG_OFFSET_DATA];  // payload 위치
            if (liked == 1)
            {
                // 후보 리스트에 추가
