...
#include <vector>  // 맨 위에 추가
static std::vector<uint8_t> tempChatCandidateList;
...
uint8_t liked_id = 2;  // 임의로 2번 노드를 좋아한다고 가정
...
void L2_FSMrun(void)
{
    ...
    switch (main_state)
    {
        case L2STATE_IDLE: //IDLE state description
            ...
            // 주기적 BCAST 전송
            if (clock() - last_broadcast_time > broadcast_interval)
            {
                last_broadcast_time = clock();

                uint8_t msg[64];
                uint8_t payload[1] = {myL2ID};  // 보낼 내 ID
